/* services-spotlight.js
 * Custom element for the calmer orbit:
 *  - distributes nodes evenly around the ring
 *  - on activation, the orbit rotates so the chosen node lands at
 *    the bottom — but more slowly and gently than services-overview
 *  - swaps the active panel below + cross-fades the centered title
 *  - keyboard nav (arrow keys), no autoplay
 *  - re-inits on shopify:section:load + block:select
 */
(function () {
  if (window.customElements && customElements.get('services-spotlight')) return;

  class ServicesSpotlight extends HTMLElement {
    constructor() {
      super();
      this.activeIndex = 0;
    ...
    connectedCallback() {
      ...
      // Cache the original titles so we can swap into the center text.
    ...
    onBlockSelect(e) {
      ...
      const target = [idx, pIdx].find((i) => i >= 0);
    ...
    getRadiusPx() {
      ...
      // Sit the nodes on the inner dashed ring (~39% from center).
      return ring * 0.39;
    }
    ...
    updateOrbit() {
      ...
      this.nodes.forEach((node, index) => {
        ...
        // Active node lands at the bottom (CSS y positive = down)
  ...
  customElements.define('services-spotlight', ServicesSpotlight);
})();
